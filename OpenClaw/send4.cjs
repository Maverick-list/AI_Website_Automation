...
async function runBroadcast() {
    const msg = fs.readFileSync(path.join(__dirname, 'message4.txt'), 'utf8');
    ...
    const photos = files.filter(f => {
        ...
        return lower.includes('broadcast') && lower.includes('4') && 
    ...
    if (photos.length > 1) {
        console.log(`Ditemukan ${photos.length} gambar untuk Broadcast 4. Menyatukan (stitching) gambar...`);
        try {
            ...
            finalMedia = path.join(projectDir, 'Broadcast_4_stitched.jpg');
            // Write as JPEG with 80% quality to compress it drastically
            ...
            console.error("Gagal menyatukan gambar.", e);
    ...
    if (finalMedia) {
        ...
        console.log("Gambar Broadcast 4 tidak ditemukan. Mengirim pesan teks saja.");
    }
    ...
    if (result.status !== 0) {
        ...
        console.log("Proses Broadcast 4 Selesai!");
    }
}
